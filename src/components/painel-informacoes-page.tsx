...
import type { PainelInformacoesListResult } from "@/lib/painel-informacoes";

type PainelInformacoesPageProps = {
  data: PainelInformacoesListResult;
  legacyResources: readonly string[];
};

function buildListHref(
  filters: PainelInformacoesListResult["filters"],
  page: number,
  per: number,
) {
  ...
  return query ? `/painel/informacao?${query}` : "/painel/informacao";
}

export function PainelInformacoesPage({
  data,
  legacyResources,
}: PainelInformacoesPageProps) {
  ...
  async function handleToggleStatus(id: number) {
    ...
    setFeedback(null);
    try {
      const response = await fetch(`/api/painel/informacao/${id}/status`, {
        method: "PATCH",
  ...
  async function handleRemove(id: number, name: string) {
    if (!window.confirm(`Deseja realmente remover a informacao ${name}?`)) {
      return;
    }
    setPendingId(id);
    setFeedback(null);
    try {
      const response = await fetch(`/api/painel/informacao/${id}/remove`, {
        method: "DELETE",
        credentials: "same-origin",
      });
      const payload = (await response.json().catch(() => null)) as
        ...
        <div className="mt-7 grid gap-8 xl:grid-cols-[minmax(0,1fr)_320px]">
                        ...
                        <tr className={rowTone} key={item.id}>
                          <td className="border border-[#d7d7d7] px-4 py-3 align-top">
                            <Link
                              ...
                              href={`/painel/informacao/${item.id}`}
                            >
                              {item.id}
                            </Link>
                          </td>
                          ...
                          <td className="border border-[#d7d7d7] px-4 py-3 align-top">
                            <div className="flex flex-wrap items-center gap-3 text-sm">
                              <Link
                                className="text-[#1868d6] underline"
                                href={`/painel/informacao/${item.id}/editar`}
                              >
                                Editar
                              </Link>
                              <button
          ...
          <aside className="grid gap-5 self-start">
            ...
            <section className="rounded-[6px] border border-[#d7e1e8] bg-white shadow-[0_10px_28px_rgba(26,61,94,0.08)]">
              ...
              <form action="/painel/informacao" className="grid gap-5 px-6 py-5" method="get">
