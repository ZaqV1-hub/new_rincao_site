...
import type { PainelCortesiaListResult } from "@/lib/painel-cortesias";

type Props = {
  data: PainelCortesiaListResult;
};

function buildHref(page: number) {
  return page > 1 ? `/painel/cortesias?page=${page}` : "/painel/cortesias";
}

export function PainelCortesiasPage({ data }: Props) {
  ...
  async function handleDelete(id: number) {
    ...
    try {
      const response = await fetch(`/api/painel/cortesias/${id}/remove`, {
        method: "DELETE",
      ...
      if (!response.ok || !payload?.ok) {
        throw new Error(
          payload?.error?.message || "Falha ao excluir a cortesia.",
        );
      }

      setFeedback(payload.data?.message || "Excluido com sucesso.");
      startTransition(() => router.refresh());
    } catch (submitError) {
      setError(
        submitError instanceof Error
          ? submitError.message
          : "Falha ao excluir a cortesia.",
      );
    }
  }
    ...
    <section className="grid gap-3">
      <div className="panel-section p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="panel-eyebrow">Cortesias</p>
                    ...
                    <td className="px-3 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <Link
                          ...
                          href={`/painel/cortesias/${item.id}/editar`}
                        >
                          Editar
                        </Link>
                        <button
