...
import { getSchoolVoucherInformation } from "@/lib/school-voucher-information";
...
export type PainelClientesListResult = {
  ...
  schoolVoucherInformation: string;
};
...
export async function listPainelClientes(
  ...
  const [typeOptions, countResult, rowsResult, pendingSchoolClassifications, schoolVoucherInformation] = await Promise.all([
    listClientTypes(),
